...
var mycaptcha = require('canvas-captcha');
...
function staticPageLogged(page, loggedGoTo) {
    return function(req, res) {
        ...
        var locale = res.locals.getLocale();

        var user = req.user;
        if (!user)
            return res.render(page, {locale: locale});
        if (loggedGoTo) 
            return res.redirect(loggedGoTo, {locale: locale});
        res.render(page, {
            user: user,
            locale: locale
        });
...
function requestDevOtt(id, callback) {
    ...
    var options = {
        url: 'https://www.bustabit.club/ott',
        include: true ,
        method: 'POST',
        'cookie': 'id='+id
    };
...
function captchaVerify(req, res, next) {
    var captcha = lib.removeNullsAndTrim(req.body['captcha']);
    var realCaptcha = lib.removeNullsAndTrim(req.session.captcha);
    if(realCaptcha != captcha)
    {
        return res.render("error-captcha");
    }
    else
    {
        next();
...
module.exports = function(app) {
    ...
    app.get('/error', function(req, res, next) { // Sometimes we redirect people to /error
        ...
        return res.render('error', {locale: locale});
    });
    ...
    app.get('/captcha', function(req, res) {
        ...
        mycaptcha(opt, function(err, data) {
            if(err) {
                res.send(err);
            }
            else {
    ...
    app.post('/sent-recover', captchaVerify, user.sendPasswordRecover);
    ...
    app.post('/login', captchaVerify, user.login);
    app.post('/register', captchaVerify, user.register);
