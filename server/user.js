...
exports.register  = function(req, res, next) {
    ...
    var locale = lib.removeNullsAndTrim(req.body.locale);
    ...
    database.createUser(username, password, email, ipAddress, userAgent, function(err, sessionId) {
        ...
        return res.redirect('/play?m=new&locale=' + locale);
    });
};
    ...
    database.validateUser(username, password, otp, function(err, userId) {
        if (err) {
            ...
            if (err === 'INVALID_OTP') {
                ...
                return res.render('login-mfa', { username: username, password: password, warning: warning, locale: locale});
            }
...
exports.security = function(req, res) {
    ...
    if (!user.mfa_secret) {
        ...
        var qrUri = 'otpauth://totp/bustabitclub:' + user.username + '?secret=' + user.mfa_potential_secret + '&issuer=bustabitclub';
...
exports.enableMfa = function(req, res, next) {
    ...
    if (otp !== expected) {
        ...
        var qrUri = 'otpauth://totp/bustabitclub:' + user.username + '?secret=' + secret + '&issuer=bustabitclub';
        user.qr_svg = qr.imageSync(qrUri, {type: 'svg'});
        user.sig = sig;
