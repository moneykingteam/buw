...
module.exports = function(opts, callback) {
	...
	var defs = {
		...
		,lines: opts.lines || defaults.lines
	}
	try {
		init()
	} catch(e) {
	...
	function init() {
		...
		for(var i = 0; i < defs.lines; i++)
		{
			ctx.beginPath();
			ctx.moveTo(0, randomBetween(0, defs.size.height));
			ctx.bezierCurveTo(randomBetween(0, defs.size.height), randomBetween(0, defs.size.height), randomBetween(0, defs.size.width), randomBetween(0, defs.size.height), defs.size.width, randomBetween(0, defs.size.height));

			ctx.fillStyle = ctx.strokeStyle = defs.strokeStyle;
			ctx.lineWidth = 2;
			ctx.stroke();
		}
