        ...
        render: function() {
            return D.div({ id: 'display-settings-container' },
                D.div({ className: 'option-row' },
                    D.span({ className: 'option-text' }, t.T('Controls size')),
                    D.select({ value: this.state.controlsSize, onChange: this._setControlsSize },
                        D.option({ value: 'big' }, t.T('big')),  
                        D.option({ value: 'small' }, t.T('small'))
                    )
                ),
                D.div({ className: 'option-row' },
                    D.span({ className: 'option-text' }, t.T('Graph mode')),
                    D.select({ value: this.state.graphMode, onChange: this._setGraphMode },
                        D.option({ value: 'graphics' }, t.T('Graphics')),
                        D.option({ value: 'text' }, t.T('Text (Low CPU usage)'))
                    )
                ),
                D.div({ className: 'option-row' },
                    D.span({ className: 'option-text' }, t.T('Player list size')),
                    D.input({
                        min: 1,
